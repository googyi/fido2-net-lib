...
public class AppleAppAttest : Fido2Tests.Attestation
{
    ...
    public AppleAppAttest()
    {
        ...
        _attestationObject = CBORObject.DecodeFromBytes(cbor);
        ...
        using (var ecdsaAtt = ECDsa.Create(eCCurve))
        {
            ...
            var cpk = CBORObject.NewMap()
                ...
                .Add(COSE.KeyTypeParameter.Crv, crv);
    ...
    public void TestAppleAppAttestMissingX5c()
    {
        ...
        () => { (AttestationType attType, X509Certificate[] trustPath) = verifier.Verify(_attestationObject["attStmt"], _authData, _clientDataJson); });
    ...
    {
        _attestationObject["attStmt"].Set("x5c", CBORObject.NewArray().Add(CBORObject.FromObject(new byte[0])).Add(CBORObject.FromObject(new byte[0])));
        var verifier = new Fido2NetLib.AppleAppAttest();
        var ex = Assert.Throws<Fido2VerificationException>(
        () => { (AttestationType attType, X509Certificate[] trustPath) = verifier.Verify(_attestationObject["attStmt"], _authData, _clientDataJson); });
