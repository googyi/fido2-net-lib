...
public class Fido2Tests
{
    ...
    public abstract class Attestation
    {
        ...
        public byte[] _rpIdHash => CryptoUtils.HashData(HashAlgorithmName.SHA256, Encoding.UTF8.GetBytes(rp));
        ...
        public byte[] _clientDataHash => CryptoUtils.HashData(HashAlgorithmName.SHA256, _clientDataJson);
        ...
        public Attestation()
        {
            _credentialID = CryptoUtils.GetRandomBytes(16);
            _challenge = CryptoUtils.GetRandomBytes(128);

            byte[] signCount = CryptoUtils.GetRandomBytes(2);
        ...
        {
            _attestationObject.Set("authData", CBORObject.DecodeFromBytes(_authData.ToByteArray()));
    ...
    public async Task TestFido2AssertionAsync()
    {
        ...
        var allowedCreds = new List<PublicKeyCredentialDescriptor>() {
            new PublicKeyCredentialDescriptor(HexConverter.StringToHex(credId.Replace("-", "")))
        };
    ...
    public void TestAppleAppAttestDev()
    {
        ...
        var json = CBORObject.DecodeFromBytes(cbor);

        var AttestationObject = new ParsedAttestationObject
        (
            fmt: json["fmt"].AsString(),
            attStmt: json["attStmt"],
            authData: AuthenticatorData.Parse(json["authData"].GetByteString())
        );

        var clientDataJson = CryptoUtils.HashData(HashAlgorithmName.SHA256, Encoding.UTF8.GetBytes("This is a test. This will need to be removed before merging."));
    ...
    public void TestAppleAppAttestProd()
    {
        ...
        var clientDataJson = CryptoUtils.HashData(HashAlgorithmName.SHA256, Encoding.UTF8.GetBytes("1234567890abcdefgh"));
    ...
    public void TestAuthenticatorData()
    {
        ...
        var rpIdHash = CryptoUtils.HashData(HashAlgorithmName.SHA256, rpId);
    ...
    public async Task TestAssertionResponse()
    {
        ...
        foreach (var (type, alg, curve) in _validCOSEParameters)
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX) && curve is COSE.EllipticCurve.P256K)
                return;
        ...
        byte[] expandedPrivateKey = null)
    {
        ...
        var hashedClientDataJson = CryptoUtils.HashData(HashAlgorithmName.SHA256, clientDataJson);
    ...
    internal static CredentialPublicKey MakeCredentialPublicKey(COSE.KeyType kty, COSE.Algorithm alg, COSE.EllipticCurve? crv, byte[] x, byte[] y, byte[] n, byte[] e)
    {
        ...
        .Add(COSE.KeyCommonParameter.KeyType, kty)
